...
const UserLogin = () => {
  ...
  const handleLoginSubmit = async (e) => {
    e.preventDefault();
    if (!email || !password) {
      setLocalError('Please fill in both email and password credentials.');
      return;
    }
    setLocalError('');
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4 py-12 sm:px-6 lg:px-8 font-sans select-none">
      <div className="max-w-md w-full space-y-8 bg-white p-8 sm:p-10 rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/40">
        ...
        <div className="text-center">
          ...
          <h2 className="text-3xl font-light text-slate-900 tracking-tight">MLX Registration Portal</h2>
          <p className="mt-2 text-sm text-slate-700 font-normal">Standard Customer Secure Sign-In</p>
        </div>
        ...
        {(localError || error) && (
          <div className="flex items-start gap-3 bg-red-50 border border-red-100 p-4 rounded-xl text-red-700 text-sm animate-pulse">
            ...
            <div>
              <span className="font-normal">Access Denied:</span> {localError || error}
            </div>
          </div>
        )}
        ...
        <div className="mt-6 text-center text-xs">
          <span className="text-slate-700 font-normal">Are you an administrator? </span>
          <Link to="/admin/login" className="font-normal text-green-600 hover:text-green-500 hover:underline transition-colors duration-150">
